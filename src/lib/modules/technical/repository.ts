...
import { deterministicTechnicalEvaluation, parseTechnicalEvaluation, type TechnicalEvaluation } from "./evaluator";
...
export async function startInterview(userId: string, role: string | null) {
  await ensureSchema();
  const existing = await pgPool.query(`SELECT id FROM technical_interviews WHERE user_id = $1 AND status = 'IN_PROGRESS' ORDER BY started_at DESC LIMIT 1`, [userId]);
  ...
  try {
    ...
    for (let i = 0; i < BASE_QUESTIONS.length; i++) {
      ...
      await client.query(`INSERT INTO technical_questions (id, interview_id, kind, prompt, sequence_no) VALUES ($1,$2,$3,$4,$5)`, [randomUUID(), id, q.kind, q.prompt, i + 1]);
    }
...
export async function getInterview(userId: string, interviewId: string) {
  await ensureSchema();
  const i = await pgPool.query(`SELECT id, role, status, started_at, completed_at FROM technical_interviews WHERE id = $1 AND user_id = $2`, [interviewId, userId]);
  if (!i.rows[0]) throw Object.assign(new Error("Technical interview not found"), { code: "NOT_FOUND" });
...
export async function answerQuestion(userId: string, interviewId: string, questionId: string, answerText: string, code?: string | null) {
  ...
  let evaluation: TechnicalEvaluation;
  try {
    ...
    evaluation = parseTechnicalEvaluation(ai.text);
  } catch {
    evaluation = deterministicTechnicalEvaluation(answerText, code);
  }

  await pgPool.query(`INSERT INTO technical_responses (id, interview_id, question_id, answer_text, code, evaluation) VALUES ($1,$2,$3,$4,$5,$6::jsonb)`, [randomUUID(), interviewId, questionId, answerText, code ?? null, JSON.stringify(evaluation)]);
...
export async function completeInterview(userId: string, interviewId: string): Promise<TechnicalResult> {
  await ensureSchema();
  const interview = await pgPool.query(`SELECT id, status FROM technical_interviews WHERE id = $1 AND user_id = $2`, [interviewId, userId]);
  if (!interview.rows[0]) throw Object.assign(new Error("Technical interview not found"), { code: "NOT_FOUND" });
  ...
  const counts = await pgPool.query(`SELECT COUNT(*)::int AS evaluated FROM technical_responses WHERE interview_id = $1 AND evaluation IS NOT NULL`, [interviewId]);
  ...
  if (answered !== total || total === 0) throw Object.assign(new Error(`Complete all ${total} questions before finishing the interview`), { code: "INCOMPLETE" });

  const responses = await pgPool.query(`SELECT evaluation FROM technical_responses WHERE interview_id = $1`, [interviewId]);
  const evaluations = responses.rows.map((r) => r.evaluation as TechnicalEvaluation).filter(Boolean);
  const avg = (key: keyof Pick<TechnicalEvaluation, "correctness" | "codeQuality" | "problemSolving" | "communication">) => Math.round(evaluations.reduce((sum, e) => sum + Number(e[key] ?? 0), 0) / evaluations.length);
  const result: TechnicalResult = {
    ...
    recommendations: ["Practice explaining trade-offs before implementation details.", "Use explicit complexity analysis when discussing algorithms.", "State assumptions and failure modes before proposing a production design."],
  };
...
function mapResult(row: Record<string, unknown>): TechnicalResult {
  return {
    overallScore: Number(row.overall_score), correctness: Number(row.correctness), codeQuality: Number(row.code_quality), problemSolving: Number(row.problem_solving), communication: Number(row.communication),
    strengths: Array.isArray(row.strengths) ? row.strengths.map(String) : [], improvements: Array.isArray(row.improvements) ? row.improvements.map(String) : [], recommendations: Array.isArray(row.recommendations) ? row.recommendations.map(String) : [],
  };
}

export async function listInterviewHistory(userId: string) {
  await ensureSchema();
  const result = await pgPool.query(`SELECT i.id, i.role, i.status, i.started_at, i.completed_at, r.overall_score, r.correctness, r.code_quality, r.problem_solving, r.communication FROM technical_interviews i LEFT JOIN technical_results r ON r.interview_id = i.id WHERE i.user_id = $1 ORDER BY i.started_at DESC LIMIT 50`, [userId]);
  return result.rows.map((row) => ({ id: row.id, role: row.role, status: row.status, startedAt: row.started_at, completedAt: row.completed_at, result: row.overall_score == null ? null : { overallScore: Number(row.overall_score), correctness: Number(row.correctness), codeQuality: Number(row.code_quality), problemSolving: Number(row.problem_solving), communication: Number(row.communication) } }));
}
