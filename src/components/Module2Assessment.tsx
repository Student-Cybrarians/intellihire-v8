...
type AssessmentAttempt = { id: string; answered: number; ability: number; sectionAbilities: Record<string, number>; currentQuestion?: AssessmentQuestion | null };
...
export function Module2Assessment() {
  ...
  const begin = async () => {
    ...
    setFeedback(null);
    try {
      const response = await fetch("/api/assessments/start", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ role: role.trim() || "Software Engineer" }) });
  ...
  useEffect(() => { const timer = window.setTimeout(() => { void begin(); }, 0); return () => window.clearTimeout(timer); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const submitAnswer = async () => {
    ...
    try {
      const response = await fetch("/api/assessments/answer", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ attemptId: attempt.id, questionId: attempt.currentQuestion.id, answerIndex: selected, responseMs: Math.max(0, Date.now() - startedAt) }) });
      const data = (await response.json()) as { state: AssessmentAttempt; correct: boolean; explanation: string; error?: { message?: string } };
      if (!response.ok) throw new Error(data.error?.message ?? "Unable to submit answer");
      setFeedback({ correct: data.correct, explanation: data.explanation }); setAttempt(data.state); setSelected(null); setStartedAt(Date.now());
    ...
    <main style={{ minHeight: "100vh", padding: "clamp(1rem, 4vw, 2rem)", background: "var(--ih-bg)", color: "var(--ih-text)", fontFamily: "var(--ih-font-body)" }}>
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 24, flexWrap: "wrap", alignItems: "end" }}>
          ...
          <div style={{ flex: "1 1 240px", maxWidth: 320 }}><label htmlFor="target-role" style={{ display: "block", fontSize: 13, marginBottom: 7 }}>Target role</label><input id="target-role" value={role} onChange={(e) => setRole(e.target.value)} disabled={busy || !!attempt?.currentQuestion || !!result} maxLength={255} style={{ width: "100%", boxSizing: "border-box", padding: ".8rem 1rem", borderRadius: 10, border: "1px solid var(--ih-surface-border)", background: "var(--ih-surface)", color: "inherit" }} /></div>
        </div>

        {error ? <div role="alert" style={{ marginTop: 20, padding: 14, borderRadius: 10, background: "#3d1313", border: "1px solid #7f2929" }}><div>{error}</div>{!attempt?.currentQuestion && !result ? <button type="button" onClick={() => void begin()} disabled={busy} style={{ marginTop: 10, padding: ".6rem .9rem", borderRadius: 8, border: "1px solid var(--ih-surface-border)", background: "var(--ih-surface)", color: "inherit", cursor: "pointer" }}>{busy ? "Retrying…" : "Retry assessment"}</button> : null}</div> : null}

        {!result && !attempt?.currentQuestion && !error ? <div style={{ marginTop: 28, padding: 24, borderRadius: 16, background: "var(--ih-surface)", border: "1px solid var(--ih-surface-border)" }}><strong>{busy ? "Preparing your assessment…" : "Assessment ready"}</strong><p style={{ color: "var(--ih-text-muted)" }}>Your first question is selected from the persisted question bank and will adapt after every answer.</p></div> : null}
        ...
        {!result && attempt?.currentQuestion ? (
          <section style={{ marginTop: 28, display: "grid", gridTemplateColumns: "minmax(0,1fr) 300px", gap: 20 }}>
            <div style={{ background: "var(--ih-surface)", border: "1px solid var(--ih-surface-border)", borderRadius: 18, padding: "clamp(18px, 4vw, 24px)" }}>
              ...
              <div style={{ display: "grid", gap: 10, marginTop: 22 }}>{attempt.currentQuestion.options.map((option, index) => <button key={`${attempt.currentQuestion?.id}-${index}`} type="button" aria-pressed={selected === index} onClick={() => setSelected(index)} disabled={busy} style={{ textAlign: "left", padding: "1rem", borderRadius: 12, border: `1px solid ${selected === index ? "var(--ih-accent)" : "var(--ih-surface-border)"}`, background: selected === index ? "rgba(74,156,255,.12)" : "transparent", color: "inherit", cursor: busy ? "not-allowed" : "pointer", opacity: busy ? .7 : 1 }}><strong>{String.fromCharCode(65 + index)}.</strong> {option}</button>)}</div>
            ...
            <aside style={{ display: "grid", gap: 14 }}>
              <Panel title="ADAPTIVE ENGINE"><p>Difficulty targets your weakest evidenced section, while ensuring every section is sampled before the engine drills down.</p></Panel>
              <Panel title="SECTION MASTERY"><div style={{ display: "grid", gap: 9 }}>{sections.map((section) => { const theta = attempt.sectionAbilities?.[section] ?? 0; const score = Math.round(Math.max(0, Math.min(100, 50 + (theta / 3) * 40))); return <div key={section}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}><span>{section}</span><strong>{score}</strong></div><div style={{ marginTop: 4, height: 5, borderRadius: 99, background: "rgba(255,255,255,.08)" }}><div style={{ width: `${score}%`, height: "100%", background: "var(--ih-accent)", borderRadius: 99 }} /></div></div>; })}</div><p style={{ fontSize: 12, marginBottom: 0 }}>These estimates are persisted with the assessment and drive the next-question selector.</p></Panel>
            </aside>
          </section>
        ) : null}

        {result ? <section style={{ marginTop: 28, display: "grid", gap: 18 }}><div style={{ background: "var(--ih-surface)", border: "1px solid var(--ih-surface-border)", borderRadius: 18, padding: "clamp(18px, 4vw, 26px)" }}><div style={{ font: "600 .7rem var(--ih-font-mono)", color: "var(--ih-accent)" }}>ASSESSMENT REPORT</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 14, marginTop: 16 }}>{[["Overall", result.overallScore], ["Accuracy", result.accuracy], ["Speed", result.speedScore]].map(([label, value]) => <div key={label as string} style={{ padding: 18, borderRadius: 12, background: "rgba(255,255,255,.04)" }}><div style={{ color: "var(--ih-text-muted)", fontSize: 12 }}>{label}</div><div style={{ fontSize: "2.2rem", fontWeight: 800, marginTop: 6 }}>{value}<span style={{ fontSize: 14, color: "var(--ih-text-muted)" }}>/100</span></div></div>)}</div></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 18 }}><Panel title="Section scores"><pre style={{ margin: 0, whiteSpace: "pre-wrap", color: "var(--ih-text-muted)" }}>{JSON.stringify(result.sectionScores, null, 2)}</pre></Panel><Panel title="Strengths"><ul>{result.strengths.map((x) => <li key={x}>{x}</li>)}</ul></Panel><Panel title="Next steps"><ul>{result.recommendations.map((x) => <li key={x}>{x}</li>)}</ul></Panel></div>{result.weaknesses?.length ? <Panel title="Focus areas"><ul>{result.weaknesses.map((x) => <li key={x}>{x}</li>)}</ul></Panel> : null}<button type="button" onClick={() => { setAttempt(null); setResult(null); setFeedback(null); setError(null); setSelected(null); void begin(); }} style={{ justifySelf: "start", padding: ".75rem 1rem", borderRadius: 10, border: "1px solid var(--ih-surface-border)", background: "var(--ih-surface)", color: "inherit", cursor: "pointer" }}>Start another assessment</button></section> : null}
      </div>
