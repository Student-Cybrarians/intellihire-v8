      ...
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 24, flexWrap: "wrap", alignItems: "end" }}>
          <div>
            ...
            <p style={{ maxWidth: 720, color: "var(--ih-text-muted)", lineHeight: 1.65 }}>
              Questions adapt to your estimated ability using a 3PL-style probability model. Every response updates your ability estimate and is persisted for recovery.
            </p>
          </div>
          <div style={{ minWidth: 240 }}>
            <label style={{ display: "block", fontSize: 13, marginBottom: 7 }}>Target role</label>
            <input value={role} onChange={(e) => setRole(e.target.value)} disabled={busy || !!result} style={{ width: "100%", padding: ".8rem 1rem", borderRadius: 10, border: "1px solid var(--ih-surface-border)", background: "var(--ih-surface)", color: "inherit" }} />
          </div>
        </div>
        ...
        {!result && attempt?.currentQuestion ? (
          <section style={{ marginTop: 28, display: "grid", gridTemplateColumns: "minmax(0,1fr) 300px", gap: 20 }}>
            <div style={{ background: "var(--ih-surface)", border: "1px solid var(--ih-surface-border)", borderRadius: 18, padding: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, fontSize: 13, color: "var(--ih-text-muted)" }}>
                <span>Question {answered + 1} of 8 · {sectionLabel}</span>
                <span>Ability {attempt.ability.toFixed(2)}</span>
              </div>
              <div style={{ marginTop: 10, height: 7, borderRadius: 999, background: "rgba(255,255,255,.08)", overflow: "hidden" }}>
                <div style={{ width: `${Math.max(5, progress)}%`, height: "100%", background: "var(--ih-accent)" }} />
              </div>
              ...
              <button type="button" onClick={submitAnswer} disabled={selected === null || busy} style={{ marginTop: 22, width: "100%", padding: ".9rem 1rem", borderRadius: 10, border: 0, background: "var(--ih-accent)", color: "var(--ih-accent-ink)", fontWeight: 800, cursor: selected === null || busy ? "not-allowed" : "pointer", opacity: selected === null || busy ? .5 : 1 }}>
                {busy ? "Evaluating…" : "Submit answer"}
              </button>
            </div>

            <aside style={{ display: "grid", gap: 14 }}>
              <div style={{ background: "var(--ih-surface)", border: "1px solid var(--ih-surface-border)", borderRadius: 16, padding: 18 }}>
                <div style={{ font: "600 .7rem var(--ih-font-mono)", color: "var(--ih-accent)" }}>ADAPTIVE ENGINE</div>
                <p style={{ color: "var(--ih-text-muted)", lineHeight: 1.55 }}>Difficulty is selected near your current ability estimate rather than from a random sequence.</p>
              </div>
              <div style={{ background: "var(--ih-surface)", border: "1px solid var(--ih-surface-border)", borderRadius: 16, padding: 18 }}>
                <div style={{ font: "600 .7rem var(--ih-font-mono)", color: "var(--ih-accent)" }}>SECTIONS</div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}>
                  {sections.map((section) => <span key={section} style={{ padding: ".55rem .7rem", borderRadius: 8, background: "rgba(255,255,255,.04)", fontSize: 12 }}>{section}</span>)}
                </div>
              </div>
            </aside>
          </section>
